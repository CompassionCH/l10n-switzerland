...
class CustomParser(models.AbstractModel):
    ...
    def parse_entry(self, ns, node):
        ...
        if len(details_nodes) == 0:
            yield transaction
            self.add_value_from_node(
                ns, node, './ns:AcctSvcrRef', transaction["narration"],
    ...
    def parse_transaction_details(self, ns, node, transaction):
        ...
        if len(found_node) != 0:
            ...
            self.add_value_from_node(ns, node, './ns:Refs/ns:AcctSvcrRef',
                                     transaction["narration"], "%s (./Refs/AcctSvcrRef)" % _("Account Servicer Reference"))
        ...
        if len(iban) > 0:
            ...
            journal = journals.filtered(
                lambda x:
                    ...
                    x.bank_acc_number.translate(trans_table) == account_number_ and
                    not x.should_qr_parsing
            )
            if journal:
