...
class GenSdWizard(models.TransientModel):
    ...
    company_car = fields.Many2one('hr.salary.rule', string='Private use of company car / N°2.2')
    deduction = fields.Many2one('hr.salary.rule', string='Total deduction / N°9')

    def gen_sd(self):
        sd_obj = self.env["hr.salary.declaration"]
        sd_obj.generate_yearly_declaration(self.date_from, self.date_to,self.company_car,
                                           self.gross_income,self.deduction,self.lpp_bvg)
